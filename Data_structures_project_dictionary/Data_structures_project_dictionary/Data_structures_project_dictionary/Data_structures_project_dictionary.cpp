...
Dictionary* fillDictList(Dictionary *dictionary) {
	...
	for (int i = 0; i < lineCount; i++) {
		//cout << "1" << endl;	////
		sWord* tmpSyn = new sWord;
		tmpSyn = nullptr;
		sWord* tmpAnto = new sWord;
		tmpAnto=nullptr;
		getline(inFile, line);
		//cout << line << endl;
		while (!line.empty()) {
		...
		while (!line.empty()) {
			...
			if (posOfHashtag == string::npos) {
				char lastAnto[30];
				strcpy(lastAnto,line.substr(0).c_str());
				insertAtTailS(tmpAnto, lastAnto);
				//cout << lastAnto<<endl; //
				//cout << tmpAnto->data << endl;
				break;
			}
...
void bubleSortD(Dictionary* dictionary) {
	dWord* cur1 = dictionary->head;
	dWord* cur2 = dictionary->head;
	dWord* tmp = new dWord;
	while (cur1 != nullptr) {
		while (cur2 != nullptr) {
			if (strcmp(cur2->synonym->data, cur2->next->synonym->data) >= 0) {
				tmp = cur2;
				cur2 = cur2->next;
				cur2->previous = tmp->previous;
